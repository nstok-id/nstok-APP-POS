      ...
      <div className="flex items-center gap-2.5 sm:gap-3.5">
        ...
        <div className="relative ml-1">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-muted/70 hover:bg-muted text-foreground border border-border text-xs font-semibold transition-all cursor-pointer"
          >
            {getArchetypeIcon(mode)}
            <span className="text-[11px] font-bold">{ARCHETYPES[mode].name}</span>
            <ChevronDown className="w-3 h-3 text-muted-foreground" />
          </button>

          {dropdownOpen && (
            <div className="absolute top-full left-0 mt-1.5 w-56 bg-card border border-border rounded-2xl shadow-2xl p-1.5 z-50 space-y-1 animate-in fade-in zoom-in-95 duration-150">
